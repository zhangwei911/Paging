...
import viz.commonlib.paging3.getValue
 ...
 * Q 网络请求字段类
 */
class ItemKeyedPagingSource<K: Serializable,D : Parcelable, N : Parcelable, Q : Parcelable>(
    ...
    override suspend fun load(params: LoadParams<K>): LoadResult<K, N> {
        return try {
            ...
            LoadResult.Page(
                    ...
                    prevKey = getValue(dataBean.items.firstOrNull(), keyMethodName) as K, // 上一页，设置为空就没有上一页的效果，这需要注意的是，如果是第一页需要返回 null，否则会出现多次请求
                    nextKey = getValue(dataBean.items.lastOrNull(), keyMethodName) as K// 下一页，设置为空就没有加载更多效果，需要注意的是，如果是最后一页返回 null
            )
        } catch (e: Exception) {
            e.printStackTrace()
            LoadResult.Error(e)
